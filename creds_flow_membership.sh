...
: "${HOLDER_DID:?Set HOLDER_DID}"

# ===== Optional env =====
DISPLAY_NAME="${DISPLAY_NAME:-Dev Issuer}"

# SCHEMA_URL="${SCHEMA_URL:-https://gist.githubusercontent.com/IvanBelyakoff/ebe89c9ebaeefa9cb38089851d7f66b8/raw/8356364aa8cdbbccdbe5427abcf64aacdfd5c43b/membership_schema.json}"
SCHEMA_URL="${SCHEMA_URL:-https://ipfs-proxy-cache.privado.id/ipfs/QmccKrYjhLJRPqWQY2Pe8f4JF6gJJFwZjzLW29nQKP76CF}"
SCHEMA_TYPE="${SCHEMA_TYPE:-OrganizationMembership}"
SCHEMA_VERSION="${SCHEMA_VERSION:-1.0}"

ORG_NAME="${ORG_NAME:-GWFM}"
EMAIL="ivan.belyakoff@example.com"

OUTDIR="${OUTDIR:-/tmp/iden3_flow_membership}"
mkdir -p "$OUTDIR"
...
ISSUER_DID="$(jq -r '.identifier // empty' "$IDENT_JSON")"
...
KEY_ID=$(curl -sS -u "$ISSUER_USER:$ISSUER_PASS" -H 'content-type: application/json' \
  ...
  "$ISSUER_URL/v2/identities/$ISSUER_DID/keys" | tee "$KEY_JSON" | jq -r '.id // empty')

if [ -z "$KEY_ID" ]; then
  echo "Create didn't return id; listing keys to reuse by name=$KEY_NAME"
  curl -sS -u "$ISSUER_USER:$ISSUER_PASS" \
    "$ISSUER_URL/v2/identities/$ISSUER_DID/keys" | tee "$KEYS_LIST_JSON" >/dev/null || true
...
if [ "$HAS_AUTH" = "true" ]; then
  ...
  curl -sS -u "$ISSUER_USER:$ISSUER_PASS" -H 'content-type: application/json' \
    ...
    "$ISSUER_URL/v2/identities/$ISSUER_DID/create-auth-credential" | tee "$AUTHCRED_JSON" | jq . || true
fi
echo
...
SCHEMA_ID=$(curl -sS -u "$ISSUER_USER:$ISSUER_PASS" -H 'content-type: application/json' \
  ...
  "$ISSUER_URL/v2/identities/$ISSUER_DID/schemas" | tee "$SCHEMA_RESP_JSON" | jq -r '.id // empty')

echo "SCHEMA_ID from POST: $SCHEMA_ID"

if [ -z "$SCHEMA_ID" ]; then
  echo "Schema id not returned; trying list match."
  curl -sS -u "$ISSUER_USER:$ISSUER_PASS" "$ISSUER_URL/v2/identities/$ISSUER_DID/schemas" \
...
LINK_ID=$(curl -sS -u "$ISSUER_USER:$ISSUER_PASS" -H 'content-type: application/json' \
  -d '{"schemaID":"'"$SCHEMA_ID"'","signatureProof":true,"mtProof":false,"limitedClaims":1,"credentialSubject":{"email":"'"$EMAIL"'","organisation":"'"$ORG_NAME"'","id":"'"$HOLDER_DID"'"}}' \
  "$ISSUER_URL/v2/identities/$ISSUER_DID/credentials/links" | tee "$LINK_RESP_JSON" | jq -r '.id // empty')
...
REQ_URI="$ISSUER_URL/v2/qr-store?id=$LINK_ID&issuer=$ISSUER_DID"
