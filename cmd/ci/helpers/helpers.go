...
func DeployContract(ctx context.Context, client *ethclient.Client, privateKey *ecdsa.PrivateKey, bytecode []byte) (*types.Receipt, error) {
	...
	tx := types.NewContractCreation(nonce, nil, gasNeeded, uint256.NewInt(gasPrice.Uint64()), bytecode)
	//tx := types.NewTransaction(nonce, common.Address{}, uint256.NewInt(0), gasNeeded, uint256.NewInt(gasPrice.Uint64()), bytecode)
