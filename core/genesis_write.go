...
func GenesisToBlock(g *types.Genesis, tmpDir string, logger log.Logger) (*types.Block, *state.IntraBlockState, *smt.SMT, error) {
	...
	go func() { // we may run inside write tx, can't open 2nd write tx in same goroutine
		...
		usingPmt := g.Config.PmtEnabledBlock != nil && g.Config.PmtEnabledBlock.Cmp(head.Number) <= 0
		normalcy := g.Config.NormalcyBlock != nil && g.Config.NormalcyBlock.Cmp(head.Number) <= 0

		if usingPmt && !normalcy {
			// Normalcy is required for PMT.
