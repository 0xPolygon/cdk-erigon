...
func TestGenesisBlockHashesZkevm(t *testing.T) {
	...
	check := func(network string) {
		...
		defer tx.Rollback()
		_, block, err := core.WriteGenesisBlock(tx, genesis, nil, false, "/tmp/"+network, logger)
...
func TestCommitGenesisIdempotency2(t *testing.T) {
	...
	logger := log.New()
	_, _, err := core.WriteGenesisBlock(tx, genesis, nil, false, "8", logger)
	require.NoError(t, err)
	seq, err := tx.ReadSequence(kv.EthTx)
	require.NoError(t, err)
	require.Equal(t, uint64(2), seq)

	_, _, err = core.WriteGenesisBlock(tx, genesis, nil, false, "9", logger)
	require.NoError(t, err)
	seq, err = tx.ReadSequence(kv.EthTx)
	require.NoError(t, err)
	require.Equal(t, uint64(2), seq)
