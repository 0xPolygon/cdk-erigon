...
func CalcBaseFeeZk(config *chain.Config, parent *types.Header) *big.Int {
	...
	if parent.Number.Cmp(big.NewInt(0)) == 0 && !config.IsSovereignModeEnabled(parent.Number.Uint64()+1) {
	...
	if parent.Number.Cmp(big.NewInt(1)) == 0 && !config.IsSovereignModeEnabled(parent.Number.Uint64()+1) {
