...
// TestNatsEmbeddedCluster tests a setup with multiple embedded NATS servers in a cluster:
// 1. Create a cluster of 3 embedded NATS servers
...
func TestNatsEmbeddedCluster(t *testing.T) {
	// Create temp directories for each server
	tempRoot, err := os.MkdirTemp("", "nats-embedded-cluster-test")
	require.NoError(t, err)
	defer os.RemoveAll(tempRoot)

	// Configure cluster with 3 nodes
	clusterName := "erigon_embedded_cluster"
	numServers := 3
