		...
		case logs := <-logChan:
			...
			for _, l := range logs {
				...
				case contracts.AddNewRollupTypeTopic, contracts.AddNewRollupTypeTopicBanana:
					...
					historyMap, err := hermezDb.GetAllForkIdBlock()
					if err != nil {
						return err
					}
					...
					} else if verifierBig.Uint64() != 0 && len(historyMap) == 0 {
						// If the chain deployment is PP, we need to fill both of them
						log.Warn("xlayer, historyMap is empty, received AddNewRollupTypeTopic", "rollupType", rollupType, "forkId", forkId)
						if funcErr = hermezDb.WriteRollupType(rollupType, forkId); funcErr != nil {
							return funcErr
						}
