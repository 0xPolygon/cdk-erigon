	...
	if l1info != nil && (l1info.Index > 0 || blockNumber == 1) {
		...
		if shouldWriteGerToContract {
			...
			if l1BlockHash == (common.Hash{}) {
				...
				log.Debug(fmt.Sprintf("[%s] Writing GER manager L1 block hash to Intra Block State", batchContext.s.LogPrefix()), "ger", l1info.GER.String(), "l1BlockHash", l1info.ParentHash.String())
