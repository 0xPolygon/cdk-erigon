...
sequencerFirstLog="${sequencerLogDir}/sequencer-initial.log"
sequencerSecondLog="${sequencerLogDir}/sequencer-halt.log"
datastreamHostLog="${sequencerLogDir}/datastream-host.log"
...
cleanup() {
    ...
    if [[ -n "${dspid:-}" ]] && kill -0 "$dspid" 2>/dev/null; then
        ...
        kill -INT "$dspid" 2>/dev/null || kill -9 "$dspid" 2>/dev/null || true
...
cast_rpc_result() {
    local response exit_code parsed
    set +e
    ...
    if (( exit_code != 0 )); then
        log_err "cast rpc error for '$*': $response"
        return 1
    fi
    ...
    if [[ "${response}" == \{* || "${response}" == \[* ]]; then
        if command -v jq >/dev/null 2>&1; then
            set +e
            parsed=$(jq -er '.result // empty' <<<"$response")
            exit_code=$?
            set -e
...
wait_for_rpc() {
    ...
    while true; do
        if block_hex=$(cast_rpc_result eth_blockNumber 2>/dev/null); then
            local block_dec=0
            if [[ -n "$block_hex" ]]; then
                block_dec=$((block_hex))
            fi
...
start_sequencer() {
    local halt_batch=${1:-}
    local log_path=${2:-"$sequencerFirstLog"}
...
wait_for_batch_reach() {
    ...
    log "Waiting for batch $target to be reached" >&2

    while (( waited < max_wait )); do
        if [[ -n "$log_path" && -f "$log_path" ]]; then
            if grep -q "Halt sequencer on batch ${target}" "$log_path"; then
                log "Halt log confirmed batch $target" >&2
                last=$target
                break
            fi
        fi
        ...
        if (( current >= target )); then
            log "Batch target reached at $current" >&2
            last=$current
            break
        fi
        sleep 1
    ...
    if (( waited >= max_wait && current < target )); then
        log_err "Timed out waiting for batch $target; last observed $current"
        exit 1
    fi
    ...
    while (( stable < stable_secs && waited < max_wait )); do
        ...
        if [[ -n "$log_path" && -f "$log_path" ]]; then
            if grep -q "Halt sequencer on batch ${target}" "$log_path"; then
                log "Halt log confirmed batch $target during stabilization" >&2
                last=$target
                stable=$stable_secs
                break
            fi
...
# trap cleanup EXIT
cleanup
trap 'log_err "Interrupted"; exit 1' SIGINT SIGTERM
...
start_sequencer "" "$sequencerFirstLog"
wait_for_block_production
...
start_sequencer "$halt_batch_target" "$sequencerSecondLog"
wait_for_rpc_port
halted_batch=$(wait_for_batch_reach "$halt_batch_target" 2 "$sequencerSecondLog")
...
go run ./zk/debug_tools/datastream-host --file="$datastreamBinaryPath" >>"$datastreamHostLog" 2>&1 &
dspid=$!
...
while ! bash -c "</dev/tcp/localhost/$datastreamHostPort" 2>/dev/null; do
    if ! kill -0 "$dspid" 2>/dev/null; then
        log_err "Datastream host process $dspid exited unexpectedly; see $datastreamHostLog"
        exit 1
    fi
